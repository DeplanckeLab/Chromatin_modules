...
def solve_cyclic_graph(G, timeout, j=0):
    G_copy = G.copy()
    G_edges_dict = {
        (edge[0], edge[1]): edge[2]["weight"] for edge in G.edges(data=True)
    }
    G_edges_dict_sorted = {
        key: G_edges_dict[key]
        for key in sorted(G_edges_dict, key=G_edges_dict.get, reverse=False)
    }
    G.remove_edge(*min(G_edges_dict.items(), key=operator.itemgetter(1))[0])
    if nx.is_weakly_connected(G) and nx.is_directed_acyclic_graph(G):
        ...
        if nx.is_weakly_connected(G_copy) and nx.is_directed_acyclic_graph(G_copy):
            return G_copy
        elif nx.is_weakly_connected(G_copy) and not nx.is_directed_acyclic_graph(
            G_copy
        ):
            if datetime.datetime.now() > timeout:
                return None
...
def find_the_heaviest_hamiltonian_path(G):
    ...
    heaviest_path_dict = {"path": None, "path_weight": 0}
...
def build_dags(
    ...
    peak_coordinates["peak_id_str"] = (
        ...
        + ":"
        + "chr"
        + peak_coordinates["chr"].astype(str)
        + ":"
    ...
    g = nx.from_pandas_edgelist(
        causality_df, "from", "to", "weight", create_using=nx.MultiDiGraph()
    )
    ...
    for index, subgraph in enumerate(list_of_subgraphs):
        ...
        start_end = sorted(
            list(zip(peak_df["start"], peak_df["end"])), key=lambda x: x[0]
        )
    ...
    dag_df = pd.DataFrame(
        (el for el in itertools.zip_longest(*list_with_dags_single_lst))
    ).T
