...
if(length(args) != 8){
    ...
    message("1: Path to Clomics executable")
    message("2: Number of peaks to the right of the focal peak")
    message("3: Dataset")
    message("4: Normalized matrices in bed.gz format from qtltools (separated by commas)")
    message("5: Output folder")
    message("ERROR: Requires 5 parameters!")
    quit(save="no")
}
param.path_to_clomics <- args[1]
param.n_peaks <- args[2]
param.dataset <- args[3]
param.matrix_files <- args[4]
param.output_folder <- args[5]
param.thresholds <- args[6]
...
param.thresholds <- strsplit(param.thresholds, split = ",")[[1]]
message(length(param.thresholds), " thresholds are found.")
# -
...
for(param.matrix_file in param.matrix_files){
    ...
    new.file <- paste0(param.output_folder, "/", param.dataset, "/", data.filename, ".clomics.bed")
...
for(chr in 1:22){
    cmd <- paste0(
      ...
      " --bed ", generated.files,
      " --out ", param.output_folder, "/", param.dataset, "/clomics.tree.chr", chr, ".out",
  ...
  for(threshold in param.thresholds){
      cmd <- paste0(
          "/software/clomics-1.0/bin/clomics call ",
          "--tree ", param.output_folder, "/", param.dataset, "/clomics.tree.chr", chr, ".out ",
          "--threshold ", threshold, " ",
          "--out ", param.output_folder, "/", "pv_", threshold, "/", param.dataset, "/clomics.tree.chr", chr, ".call.out"
      )
      message("Running command: ", cmd)
      system(command = cmd)

      cmd <- paste0(
          "/software/clomics-1.0/bin/clomics locate --bed ", generated.files, " ",
          "--tree ", param.output_folder, "/", "pv_", threshold, "/", param.dataset, "/clomics.tree.chr", chr, ".call.out ",
          "--out ", param.output_folder, "/", "pv_", threshold, "/", param.dataset, "/clomics.tree.chr", chr, ".final.out"
      )
      message("Running command: ", cmd)
      system(command = cmd)
    }
...
for(threshold in param.thresholds){
    ...
    fwrite(
        data.clomics,
        paste0(param.output_folder, "/", "pv_", threshold, "/", param.dataset, "/CRD.all.clomics.bed"),
        sep = "\t",
        col.names = F,
        row.names = F,
        na = "NA",
...
for(threshold in param.thresholds){
    ...
    cmd <- paste0(
        "python3.9 /data/pushkare/computational_paper/GITHUB/00.CM_mapping/Clomics/main/CRDs_to_CMs_parallel.py",
        " -d ", param.dataset,
        " -i ", param.output_folder, "/", "pv_", threshold, "/", param.dataset, "/CRD.all.clomics.bed ",
        " -o ", param.output_folder, "/", "pv_", threshold, "/", param.dataset,
        " -p ", 50
    )
    message("Running command: ", cmd)
    system(command = cmd)
