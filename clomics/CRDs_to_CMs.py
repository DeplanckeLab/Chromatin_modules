...
peak_id_to_check = (
    crd_tree_df[crd_tree_df.loc[:, "first_child"] == "nan"].loc[0, "UID"].copy()
)
...
if peak_id_to_check.startswith("chr"):
    ...
    crd_content = pd.DataFrame(
        columns=["CM_id", "CM_size", "peaks"], index=np.arange(crd_nodes.shape[0])
    )
    for index, crd in tqdm.tqdm(crd_nodes.iterrows()):
        ...
        crd_content["CM_id"].loc[index] = "crd" + str(index)
        crd_content["CM_size"].loc[index] = int(len(list_with_marks))
        ...
        start_end = sorted(
            [
                (
                    int(crd_tree_df[crd_tree_df["UID"] == mark]["start"]),
                    int(crd_tree_df[crd_tree_df["UID"] == mark]["end"]),
                )
        ...
        bed_df["CM_size"].loc[index] = int(len(list_with_marks))
    ...
    bed_df.to_csv(
        os.path.join(path_to_output_directory, dataset + "Clomics_CM.tracks.bed"),
        sep="\t",
        index=False,
        header=False,
    )
    crd_content.to_csv(
        os.path.join(path_to_output_directory, dataset + "Clomics_CM.content.txt"),
        sep="\t",
        header=False,
        index=False,
    )
