...
from DAG2VCM_class import DAG2VCM

parser = argparse.ArgumentParser(
    description='Build VCM-like track and content files from DAGs ©Olga Pushkarev')
parser.add_argument(
    '-d',
    '--dataset',
    type=str,
...
logging.info('####################################################################')
logging.info('# Build VCM-like track and content files from DAGs ©Olga Pushkarev #')
logging.info('####################################################################')
logging.info('\nList of arguments:\n')
logging.info('Input:')
...
tracks_content_path = os.path.join(
    ...
    '_'.join([dataset, 'phm_tracks_content'])
)
...
DAG2VCM_class = DAG2VCM(path_to_output_directory, chromosomes, pp_threshold, dataset)
logging.info('Merging data from all chromosomes...')
# merged_tracks = DAG2VCM_class.merge_tracks()
# merged_contents = DAG2VCM_class.merge_contents()
# merged_dags = DAG2VCM_class.merge_dags()
...
#     os.path.join(
#         DAGs_path,
#         '_'.join([str(pp_threshold), 'merged_DAGs_for_all_chr.tsv'])
#     ),
#     sep='\t',
#     index=False,
#     header=False
...
#     tracks_column_names = ['chr', 'start', 'end', 'cm_id', 'number', 'strain',
#                             'start_duplicate', 'end_duplicate', 'numbers', 'cm_size',
#                             'peak_length', 'peak_starts', 'is_totem']
#     merged_tracks.columns = tracks_column_names
#     merged_tracks[merged_tracks['is_totem'] == 0].to_csv(
#         os.path.join(
#             tracks_content_path,
#             '_'.join([dataset, str(pp_threshold), 'merged_phm_all_chr_NOT_TOTEM.tracks.bed'])
#             ),
#         sep='\t',
#         index=False,
#         header=False)
...
#     not_totem_contents = merged_contents[merged_contents['cm_id'].isin(
#         merged_tracks[merged_tracks['is_totem'] == 0]['cm_id'])]
#     not_totem_contents.to_csv(
#         os.path.join(
#             tracks_content_path,
#             '_'.join([dataset, str(pp_threshold), 'merged_phm_all_chr_NOT_TOTEM.content.txt'])
#             ),
#         sep='\t',
#         index=False,
#         header=False
...
#     path_to_output_directory,
#     '_'.join([dataset, 'all_chr_peak_coordinates.tsv'])
# )
...
if not os.path.exists(lead_variants_path):
    ...
    logging.info('File with lead variants is missing! Preparing and saving lead variants...')
    for chromosome in chromosomes:
        ...
        all_variants_df.columns = ['Peak', 'Chr', 'Pos', 'RsID', 'Ref', 'Alt', 'AF', 'R2',
                                    'Variant_loc', 'Inside_Peak', 'Beta', 'SE', 'Log10_BF']
        hm_pp = pd.read_csv(
        ...
        peaks_and_lead_snps = pd.DataFrame(
            columns=['Peak', 'chr', 'rs_pos', 'RsID', 'Ref_Alt',
                    'AF', 'R2', 'rs_location', 'inside_peak',
                    'Beta', 'SE', 'Log10_BF', 'pp_of_caQTL'],
        ...
        for idx, (peak_id, peak_df) in enumerate(all_variants_df.groupby('Peak')):
            ...
            max_pp_variants = rs_peaks_chr[
                rs_peaks_chr['pp_of_causal_QTL'] == rs_peaks_chr['pp_of_causal_QTL'].max()
            ]
            if max_pp_variants.shape[0] == 0:
                continue
            rs_coord_pair = sorted(list(zip(max_pp_variants['RsID'].astype(str), max_pp_variants['Pos'])))
            rs_ids = ';'.join([rs[0] for rs in rs_coord_pair])
            coordinates = ';'.join([str(pos[1]) for pos in rs_coord_pair])

            rsloc_inpeak_pair = sorted(list(zip(max_pp_variants['Variant_loc'], max_pp_variants['Inside_Peak'])))
            rs_loc = ';'.join([str(rloc[0]) for rloc in rsloc_inpeak_pair])
            in_peak = ';'.join([str(ip[1]) for ip in rsloc_inpeak_pair])

            ref_alt_pair = sorted(list(zip(max_pp_variants['Ref'], max_pp_variants['Alt'])))
            ref_alt_str = ';'.join([str(ref_alt[0]) + ':' + str(ref_alt[1]) for ref_alt in ref_alt_pair])

            peaks_and_lead_snps['Peak'].loc[idx] = str(peak_id) + '_chr' + str(list(max_pp_variants['Chr'].values)[0])
            ...
            peaks_and_lead_snps['Beta'].loc[idx] = list(max_pp_variants['Beta'].values)[0]
            peaks_and_lead_snps['SE'].loc[idx] = list(max_pp_variants['SE'].values)[0]
            peaks_and_lead_snps['Log10_BF'].loc[idx] = list(max_pp_variants['Log10_BF'].values)[0]
            peaks_and_lead_snps['pp_of_caQTL'].loc[idx] = list(max_pp_variants['pp_of_causal_QTL'].values)[0]
        ...
        lead_variants_df.columns = ['Peak', 'Chr', 'Pos', 'RsID', 'Ref', 'Alt', 'AF', 'R2',
                                    'P_Lead', 'Inside_Peak', 'Beta', 'SE', 'Log10_BF']
        lead_variants_unique = lead_variants_df.loc[lead_variants_df.groupby('Peak')['Log10_BF'].idxmax()]
        ...
        hm_pp_max_pp = hm_pp.groupby('peak_id').agg({'pp_of_causal_QTL': 'max'})['pp_of_causal_QTL'].reset_index()
