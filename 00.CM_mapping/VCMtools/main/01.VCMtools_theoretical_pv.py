...
def process_different_marks_pv(path_to_output_directory,
                               dataset,
                               dict_for_chr,
                               max_dist=0.5 * (10 ** 6)):
    with open(os.path.join(
        ...
        for peak1, peak1_values in dict_for_chr.items():
            ...
            for peak2, peak2_values in dict_for_chr.items():
                ...
                try:
                    if (peak1 != peak2) and (abs(peak1_center - peak2_center) <= max_dist):
                        ...
                        file_with_corr.write('\t'.join([peak1,
                                                        peak2,
...
if path_to_input_directory:
    if (dataset == 'iPSCs'):
        dict_with_count_matrices['ATAC'] = 'ATAC_iPSC_iPSCmeta_only_PCs16_RPKMnorm_regressed_qqnorm_forCRD.bed.gz'
    elif (dataset == 'Gaffney'):
        dict_with_count_matrices['ATAC'] = 'ATAC_Gaffney_gafmeta_only_16PCs_RPKMnorm_regressed_qqnorm_forCRD.bed.gz'
    else:
...
for mark, mark_count_mtx_path in dict_with_count_matrices.items():
    if path_to_input_directory:
        if (dataset == 'iPSCs') or (dataset == 'Gaffney'):
            df = pd.read_csv(os.path.join(path_to_input_directory, mark_count_mtx_path),
                            sep='\t')
            df['#Chr'] = df['#Chr'].str.replace('chr', '')
        else:
            df = pd.read_csv(os.path.join(path_to_input_directory, dataset, mark_count_mtx_path),
                            sep='\t')
    elif input_files:
...
for chromosome in chromosomes:
    all_data_df_chr = pd.concat([mark_df[mark_df['#Chr'] == chromosome]
...
with mp.Pool(processes=int(n_cores)) as pool:
    ...
    proc_results = [pool.apply_async(process_different_marks_pv,
                                     args=(path_to_output_directory,
                                           dataset,
                                           chunk))
