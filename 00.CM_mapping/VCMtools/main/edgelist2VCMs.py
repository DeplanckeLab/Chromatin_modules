...
class edgelist2VCMs:
        ...
        for pair in sorted(list_of_pairs):
            if not merged_intervals:
                ...
                if pair[0] <= lower_pair[1]:
                    ...
                    merged_intervals[-1] = (lower_pair[0], upper_bound)  # replace by merged interval
                else:
                    merged_intervals.append((pair[0], pair[1]))
    ...
    def get_VCMs(
        ...
        connected_components = self.connected_tuples(
            list(
                zip(
                    pv_filtered_corr_edgelist.peak1,
                    pv_filtered_corr_edgelist.peak2)
                )
            )
        bed_df = pd.DataFrame(
            columns=[
                'chr', 'start', 'end', 'VCM_id', 'number', 'strain',
                'start_duplicate', 'end_duplicate', 'numbers', 'VCM_size',
        ...
        for index, vcm in tqdm.tqdm(enumerate(connected_components)):
            ...
            bed_df['totem_VCM'].loc[index] = 1 if len(self.merge_intersecting_intervals(start_end)) == 1 else 0
        ...
        if self.remove_totem:
            ...
            final_vcm_content = sorted_vcm_content[sorted_vcm_content.loc[:, 'VCM_id'].isin(final_vcm_tracks.loc[:, 'VCM_id'])]
        else:
        ...
        if self.save_meta:
            if len(connected_components) != 0:
                ...
                meta_dict['mean_abs_corr'] = np.mean(abs(pv_filtered_corr_edgelist['corr']))
                meta_dict['VCM_lengths'] = list(final_vcm_tracks.loc[:, 'end'] - final_vcm_tracks.loc[:, 'start'])
